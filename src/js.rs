                ...
                Expression::Word(op) => match op.as_str() {
                  "do" => {
                      ...
                      let mut compiled: Vec<String> =
                          items[1..].iter().map(|e| compile_expr_to_js_inner(e, false)).collect();

                      let last = compiled.pop().unwrap();
                      let mut stmts: Vec<String> = compiled.into_iter().map(|s| format!("{};", s)).collect();
                  ...
                  "get" =>  format!("({}[{}])",  compile_expr_to_js(&items[1]),  compile_expr_to_js(&items[2])),
                  "set!" =>  format!("({}[{}]={},0)",  compile_expr_to_js(&items[1]),  compile_expr_to_js(&items[2]), compile_expr_to_js(&items[3])),
                  "pop!" =>  format!("({}.pop(),0)",  compile_expr_to_js(&items[1])),
                  "*" =>  format!("({} * {})",  compile_expr_to_js(&items[1]),  compile_expr_to_js(&items[2])),
                  "+" =>  format!("({} + {})",  compile_expr_to_js(&items[1]),  compile_expr_to_js(&items[2])),
                  "-" =>  format!("({} - {})",  compile_expr_to_js(&items[1]),  compile_expr_to_js(&items[2])),
                  "/" =>  format!("(({} / {}) | 0)",  compile_expr_to_js(&items[1]),  compile_expr_to_js(&items[2])),
                  "mod" =>  format!("({} % {})",  compile_expr_to_js(&items[1]),  compile_expr_to_js(&items[2])),
                  ">" =>  format!("({} > {})",  compile_expr_to_js(&items[1]),  compile_expr_to_js(&items[2])),
                  "<" =>  format!("({} < {})",  compile_expr_to_js(&items[1]),  compile_expr_to_js(&items[2])),
                  ">=" =>  format!("({} >= {})",  compile_expr_to_js(&items[1]),  compile_expr_to_js(&items[2])),
                  "<=" =>  format!("({} <= {})",  compile_expr_to_js(&items[1]),  compile_expr_to_js(&items[2])),
                  "=" =>  format!("({} == {})",  compile_expr_to_js(&items[1]),  compile_expr_to_js(&items[2])),
                  "or" =>  format!("({} || {})",  compile_expr_to_js(&items[1]),  compile_expr_to_js(&items[2])),
                  "and" =>  format!("({} && {})",  compile_expr_to_js(&items[1]),  compile_expr_to_js(&items[2])),
